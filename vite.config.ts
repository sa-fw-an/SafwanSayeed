    ...
    tags: [
      {
        ...
        attrs: {
          ...
          content: [
            ...
            // The 3D pipeline: Draco decodes in a blob: worker, and
            // GLTFLoader fetches/unwraps model textures as blob: URLs.
            "worker-src 'self' blob:",
            "child-src 'self' blob:",
            // blob: — GLTFLoader unwraps the room model's embedded textures
            // as blob: URLs; without it every texture 404s under CSP.
            "img-src 'self' blob: data: https://hits.sh",
